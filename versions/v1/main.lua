...
-- Save/load checkpoint index
local function saveProgress(idx)
  love.filesystem.write(params.save_file, tostring(idx))
end
...
function love.load()
  ...
  currentCheckpoint = loadProgress()
...
function love.update(dt)
  ...
  for i=1,len do
    ...
    if col.normal.x ~= 0 and not player.onGround and math.abs(player.vx) > params.wall_knockback_min_speed then
      ...
      player.vx = -col.normal.x * knockbackForce
    end
      ...
      for idx,cp in ipairs(checkpoints) do
        if cp == col.other then
          if currentCheckpoint ~= idx then
            ...
            saveProgress(currentCheckpoint)
          end
        end
      end
    end
  ...
  local targetCamX = player.x + player.w/2
  local targetCamY = player.y - params.cam_anchor_below

  -- lookahead when fully charged
  local chargeRatio = clamp((player.t_charge - params.t_min) / (params.t_max - params.t_min), 0, 1)
  if player.charging then
    targetCamY = targetCamY - params.lookahead_fullcharge * chargeRatio
  end

  local smooth = params.cam_smooth
  cam.x = lerp(cam.x, targetCamX, math.min(1, dt / smooth))
...
function love.draw()
  ...
  -- camera translation (same as before)
  love.graphics.push()
  love.graphics.translate(sx/2 - cam.x, sy/2 - cam.y)

  -- 1) draw tilemap (STI) if available
  if tilemap and tilemap.draw then
    -- STI draw draws visible layers in their order
    tilemap:draw()
  end
  ...
  -- 3) draw moving platforms (they are already in platforms list; above covers them if not fromTiled)
  -- (no extra code needed unless you want a special color)

  -- 4) draw player
  love.graphics.setColor(1, 0.9, 0.2, 1)
  love.graphics.rectangle('fill', player.x, player.y, player.w, player.h)

  -- landing indicator (keeps being visible)
  if player.onGround then
